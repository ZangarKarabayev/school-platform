    ...
    <section style="padding-bottom:24px;">
        <div style="background:#fff;border:1px solid #d1d8e5;border-radius:20px;box-shadow:0 12px 32px rgba(35,64,103,0.08);overflow:hidden;">
            @if ($terminals->isEmpty())
                <div style="padding:24px;color:#71829a;">{{ __('ui.devices_page.empty') }}</div>
            @else
                    ...
                    <table style="width:100%;border-collapse:collapse;min-width:900px;">
                        <thead>
                            <tr style="background:#f7f9fc;color:#4e607d;text-align:left;">
                                ...
                                <th style="padding:14px 16px;">{{ __('ui.devices_page.last_heartbeat') }}</th>
                                <th style="padding:14px 16px;">{{ __('ui.devices_page.status') }}</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($terminals as $terminal)
                                ...
                                <tr style="border-top:1px solid #e4e9f1;">
                                    ...
                                    <td style="padding:14px 16px;">
                                        <span style="display:inline-flex;align-items:center;gap:8px;padding:6px 10px;border-radius:999px;background:{{ $isOnline ? __('ui.devices_page.online') : __('ui.devices_page.offline') }};color:{{ $isOnline ? '#167c3b' : '#c2414c' }};font-weight:700;font-size:13px;">
                                            <span style="width:8px;height:8px;border-radius:50%;background:currentColor;"></span>
                                            {{ $isOnline ? __('ui.devices_page.online') : __('ui.devices_page.offline') }}
                                        </span>
                                    </td>
                                </tr>
                            @endforeach
